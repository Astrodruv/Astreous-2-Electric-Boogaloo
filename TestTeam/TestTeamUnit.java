...
//        currentTarget = nearestEnemyThreat;
    ...
    public void calculateEnemyAttackType(){
        ...
        if (((float) lightEnemyThreats / enemyThreats.size()) > 0.4f){ // if more than 50% of all attacking units are Light
            enemyAttackScheme = "Very Light";
        }
        else if (((float) mediumEnemyThreats / enemyThreats.size()) > 0.4f){
            enemyAttackScheme = "Very Medium";
        }
        else if (((float) heavyEnemyThreats / enemyThreats.size()) > 0.4f){
            enemyAttackScheme = "Very Heavy";
        }
        else if (((float) assaultEnemyThreats / enemyThreats.size()) > 0.4f){
        ...
        if (((float) lightEnemyThreats / enemyThreats.size()) > 0.2f && ((float) mediumEnemyThreats / enemyThreats.size()) > 0.2f){
            enemyAttackScheme = "Light-Medium Rush";
        }
        else if (((float) mediumEnemyThreats / enemyThreats.size()) > 0.2f && ((float) heavyEnemyThreats / enemyThreats.size()) > 0.2f){
    ...
    public void calculateWorkersInDanger(){
        ...
        if (myWorkers.contains(this)){
            if (getNearestEnemyThreat() != null && getDistance(getNearestEnemyThreat()) > getNearestEnemyThreat().getMaxRange() * 2f){
    ...
    public void calculateEnemyWorkerStrength() {
        ...
        if ((float) enemyWorkers.size() / getEnemiesExcludeBaseShip().size() > 0.6f) {
            enemyWorkerStrength = "High";
        }
        else if ((float) enemyWorkers.size() / getEnemiesExcludeBaseShip().size() > 0.4f) {
            enemyWorkerStrength = "Medium";
        }
        else if ((float) enemyWorkers.size() / getEnemiesExcludeBaseShip().size() > 0.2f) {
    ...
    public void calculateEnemyMissiles(){
        ...
        else if ((float) enemyMissiles.size() / enemyThreats.size() > 0.15f) {
    ...
    public void calculateMainPushStrength(){
        ...
        else if (myAttackers.size() > enemyThreats.size() * 1.2f){
        ...
        else if (myAttackers.size() * 1.2f < enemyThreats.size()){
            myMainPushStrength = "Low";
        }
        else if (myAttackers.size() * 1.5 < enemyThreats.size()){
    ...
    public void calculateMainPushDensity(){
        if (myAttackers.contains(this)){
            ...
            else if (alliesInRadius < myAttackers.size() * 0.8f){
                myMainPushDensity = "Decent";
            }
            else if (alliesInRadius < myAttackers.size() * 0.5f){
                myMainPushDensity = "Moderate";
            }
            else if (alliesInRadius < myAttackers.size() * 0.3f){
    ...
    public void setAttackState(){
        if (teamAlert.equals("High") || teamAlert.equals("Very High") || teamAlert.equals("Extreme")){
            if (enemyAttackScheme.equals("Very Light")){
                ...
                if (myMainPushStrength.equals("Very Low") || myMainPushStrength.equals("Low") || myMainPushStrength.equals("Moderately Low")) {
                    attackState = "Defense";
                }
                else if (myMainPushStrength.equals("High") || myMainPushStrength.equals("Moderate") || myMainPushStrength.equals("Moderately High")){
                    attackState = "Attack";
                }
        ...
        if (myMainPushStrength.equals("Very High") || myMainPushStrength.equals("High") || myMainPushStrength.equals("Moderately High")){
            attackState = "Attack";
        }
        else{
//            attackState = "Growth";
            if (teamAlert.equals("Low") || teamAlert.equals("Very Low")){
                attackState = "Growth";
            }
            else {
                attackState = "Defense";
        ...
        //shouldn't retreat if less than 3 units go to our base
